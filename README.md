...
デフォルトで測定ID `G-R5DGPBMER4` が設定されています。ページビューと予約ボタンクリック（`reservation_click`）を計測します。別のプロパティを使う場合は `.env.local` または Vercel の環境変数で `NEXT_PUBLIC_GA_MEASUREMENT_ID` を上書きできます。
